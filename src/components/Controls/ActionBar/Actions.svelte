<script>
	...
	// 答案提示
	function handleAnswerHint() {
		const { x, y } = $cursor;
		if (x === null || y === null) return;
		gameStore.applyAnswerHint(y, x);
	}
...
<div class="action-buttons space-x-3">
	...
	<button 
		...
		disabled={$gamePaused || !hintsAvailable || $cursor.x === null} 
		on:click={handleAnswerHint} 
		title="Answer Hint ({$hints})"
	>
